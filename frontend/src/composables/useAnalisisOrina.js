...
export const DEFAULT_ANALYTES = [
  {
    key: 'PH Orina',
    labelKey: 'urine_ph',
    aliases: ['PH Orina', 'ph orina', 'pH orina', 'ph'],
    unit: 'pH',
    validationMin: 0,
    validationMax: 14,
    decimals: 1,
...
/**
 * Composable para gestionar el análisis de orina del historial clínico (pH, etc.).
 * Cada llamada crea una instancia de estado propia, igual que useAnalisisSangre.
 */
export function useAnalisisOrina() {
  ...
  async function load() {
    ...
    if (!dto.analisisOrina) {
      entries.value = []
      return
    }

    try {
      const parsed = typeof dto.analisisOrina === 'string' ? JSON.parse(dto.analisisOrina) : dto.analisisOrina
  ...
  async function addEntry(payload) {
    ...
    saving.value = true
    try {
      await svc.añadirAnalisisOrina(body)
      await load()
    } catch (e) {
      await load()
      throw e
