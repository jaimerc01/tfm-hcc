<template>
  <div class="page">
    ...
    <p v-else>Acceso verificado para rol ADMINISTRADOR.</p>
  </div>
</template>
...
export default {
  ...
  async created() {
    try {
      ...
      const { data } = await axios.get(base + '/admin/ping', { headers: { Authorization: `Bearer ${token}` } })
      this.alive = data === 'OK-ADMIN'
    } catch (e) {
